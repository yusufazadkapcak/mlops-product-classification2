    ...
    def _clean_old_checkpoints(self):
        ...
        for checkpoint in checkpoints:
            if checkpoint not in checkpoints_to_keep and checkpoint != best_path:
                checkpoint.unlink()
                metadata_path = self.checkpoint_dir / f"{checkpoint.stem}_metadata.joblib"
                if metadata_path.exists():
                    metadata_path.unlink()
                print(f"Removed old checkpoint: {checkpoint}")


def create_checkpoint_callback(checkpoint_manager: ModelCheckpoint, validation_metric: str = "val_accuracy"):
    """
    ...
    def callback(env):
        ...
        if iteration > 0 and iteration % checkpoint_manager.save_freq == 0:
            ...
            if validation_metric in metrics:
                ...
                if checkpoint_manager.best_score is None or current_score > checkpoint_manager.best_score:
